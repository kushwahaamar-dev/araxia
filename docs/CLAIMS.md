...
| The Fitbit Air keeps streaming its last value after removal; Araxia detects this by variation analysis and flips STALE after 30 s frozen. Measured, not assumed. | `capture/hardware_gate.md`, `capture/hr_offwrist.jsonl`, `tests/test_presence.py` | Skin-contact sensing (the device reports it unsupported). |
