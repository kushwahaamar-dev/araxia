...
export function BankingDemo() {
  ...
  const receiptMessage = result?.outcome === "DENIED" ? result.reason : result?.outcome === "EXECUTED" ? (result.status === "CONFIRMED" ? "The Nessie sandbox accepted your transfer request." : result.note ?? "The transfer was not confirmed.") : "The transfer was not confirmed.";
  ...
  const execute = async () => {
    ...
    setStage("receipt");
    if (r.status === 0) setError("The transfer result is uncertain. Do not retry; check Nessie before taking action.");
  };
    ...
    <div className="banking-shell">
      <header className="banking-header">
        <div className="banking-header-inner">
          <Link href="/" className="brand-lockup" aria-label="Capital One Sandbox home"><span className="brand-mark" aria-hidden="true" /> <span>Capital One <span className="brand-muted">Sandbox</span></span></Link>
          <div className="header-meta"><span className="nessie-label">NESSIE DEMO</span><span className="header-user">Amar K. <span className="avatar">AK</span></span></div>
        </div>
      </header>
      ...
      <main className="banking-main">
        ...
        <div className="banking-grid">
          <section className="transfer-card" aria-label="Transfer workflow">
            ...
            {stage === "receipt" && <div className="flow-panel receipt-panel"><div className={receiptStatus === "CONFIRMED" ? "receipt-icon confirmed" : "receipt-icon"} aria-hidden="true">{receiptStatus === "CONFIRMED" ? "✓" : "!"}</div><p className="section-kicker">{receiptStatus === "CONFIRMED" ? "Transfer complete" : receiptStatus === "UNCERTAIN" ? "Transfer status uncertain" : "Transfer not completed"}</p><h2>{receiptStatus === "CONFIRMED" ? "Your transfer is on its way" : receiptStatus === "UNCERTAIN" ? "Check Nessie before retrying" : "We couldn&apos;t complete that transfer"}</h2><p className="muted">{receiptMessage}</p><dl className="receipt-details"><div><dt>Amount</dt><dd>{created ? dollars(created.action.amount_minor, created.action.ccy) : money(amount)}</dd></div><div><dt>Recipient</dt><dd>{recipientName}</dd></div><div><dt>Date and time</dt><dd>{new Date().toLocaleString()}</dd></div>{result && result.outcome === "EXECUTED" && result.providerRef && <div><dt>Nessie reference</dt><dd>{result.providerRef}</dd></div>}</dl><div className="form-actions"><a className="bank-button button-link" href={inspectHref}>Inspect authorization</a><button className="text-button" onClick={reset}>Make another transfer</button></div></div>}
          </section>

          <aside className="side-column"><section className="status-card"><div className="status-card-top"><span className={`status-dot ${readiness.tone}`} /><span className="section-kicker">Araxia status</span></div><h2>{readiness.title}</h2><p>{readiness.detail}</p><div className="status-line"><span>Wearable stream</span><StateBadge value={status?.evidence?.presence ?? "WAITING"} tone={status?.evidence ? toneFor(status.evidence.presence) : "off"} /></div><div className="status-line"><span>Passkey</span><span className="status-value">{passkeys.length ? "Registered" : "Not registered"}</span></div><div className="status-line"><span>Nessie executor</span><span className="status-value">{status?.rails?.nessie ? "Available" : "Unavailable"}</span></div>{!status?.rails?.nessie && <p className="executor-note">Configure NESSIE_API_KEY on the server to post a sandbox transfer.</p>}<a href={inspectHref} className="side-link">View security details <span aria-hidden="true">→</span></a></section><section className="activity-card"><div className="activity-heading"><h2>Recent activity</h2><a href={inspectHref}>See all</a></div>{status?.executions.length ? status.executions.slice(0, 3).map((item) => <div className="activity-row" key={item.id}><span className="activity-icon">↗</span><div><strong>{dollars(item.action.amount_minor, item.action.ccy)} to {item.action.dst === "demo_rent" ? "RENT" : "recipient"}</strong><small>{new Date(item.started_at).toLocaleDateString()} · {item.status.toLowerCase()}</small></div></div>) : <p className="empty-activity">No transfers yet. Your completed transfers will appear here.</p>}</section><div className="disclaimer"><strong>Sandbox account</strong><p>This is a fictional demo using Capital One Nessie. No real money is moved.</p></div></aside>
        </div>
      </main>
    </div>
  );
